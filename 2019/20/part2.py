...
tmp = {}
innerportals = set()
def addportal(wallx, wally, floorx, floory, inner=False):
    global aa, zz
    if wallx == floorx:
        if wally < floory:
...
for i in range(minx, maxx):
    if m[miny][i] != ' ':
        addportal(i, miny, i, miny-1, True)
    if m[maxy-2][i] != ' ':
        addportal(i, maxy-1, i, maxy, True)
for i in range(miny, maxy):
    if m[i][minx] != ' ':
        addportal(minx, i, minx-1, i, True)
    if m[i][maxx-2] != ' ':
        addportal(maxx-1, i, maxx, i, True)

generation = 0
walls = {(x, y) for y, i in enumerate(m) for x, j in enumerate(i) if j == '#'}
known_states = set()
next_states = {aa+(0,)}
prev_states = set()
while zz+(0,) not in known_states:
    prev_states, next_states = next_states, set()
    generation += 1
    assert len(prev_states) > 0
    for x, y, level in prev_states:
        for ns in ((x, y-1), (x, y+1), (x-1, y), (x+1, y)):
            nlevel = level
            if ns in portals:
                if ns in innerportals:
                    nlevel += 1
                elif level:
                    nlevel -= 1
                else:
                ...
                ns = portals[ns]
            ns += (nlevel,)
            if ns not in known_states and ns[:2] not in walls:
                next_states.add(ns)
                known_states.add(ns)
print(generation)

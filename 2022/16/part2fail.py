...
while len(stack) > 0 or i < len(nodes):
    ...
    if ns not in visited:
        if time[e] + dists[(s[e], ns)] + 1 <= 26:
            stack.append((s[0], s[1], i+1, value, time[0], time[1]))
            visited.append(ns)
            time[e] += dists[(s[e], ns)] + 1
            s[e] = ns
            value += rates[ns]*(26-time[e])
            if value > maxvalue:
                maxvalue = value
                print(visited)
                print(maxvalue)
    ...
    while i == len(nodes):
        if len(stack) == 0:
            break
        s[0], s[1], i, value, time[0], time[1] = stack.pop()
        visited.pop()
print(maxvalue)

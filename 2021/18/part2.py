...
def copytree(tree):
    if isinstance(tree, Pair):
        return Pair(copytree(tree.lhs), copytree(tree.rhs))
    else:
        return Number(tree.val)
def sumtree(a, b):
    tree = Pair(copytree(a), copytree(b))
    reduction(tree)
    return tree
print(max(magnitude(sumtree(trees[i], trees[j])) for i in range(len(trees)) for j in range(len(trees)) if i != j))
